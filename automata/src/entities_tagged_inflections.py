...
class EntitiesTaggedInflections(ABC):
    def __init__(
        self, infile: str, outfile: str, eti_mode: EtiMode = EtiMode.BASIC
    ) -> None:
        ...
        self.set_lang()
        self.eti_mode = eti_mode
        if not os.path.exists(self.outdir):
            os.makedirs(os.path.exists)

    def process(self) -> None:
        process_command = self.get_process_command()
        if process_command:
            process_command.split()[0]
            with open(self.outlogfile, "wb") as f_log, open(
                self.outerrfile, "wb"
            ) as f_err:
                ps = subprocess.Popen(
                    shlex.split(process_command), stdout=f_log, stderr=f_err
                )
                ps.communicate()
                ...
                if retcode == 2:
                    with open(self.outerrfile, "r") as f_tmp:
                        for ferr_lastline in f_tmp:
                            pass
                    print(
                        f"ERROR in tagged inflections of entities: Bad filepath of executable - failed with return code {retcode}.\nDetail: {ferr_lastline.strip()}.\n(command: {process_command})",
                        file=sys.stderr,
                    )
                    sys.exit(10)
                elif retcode:
                    print(
                        f"ERROR in tagged inflections of entities: Execution of following command failed with return code {retcode}. For details see error log in {self.outerrfile}.\n(command: {process_command})",
                        file=sys.stderr,
                    )
                    sys.exit(10)
            if self.eti_mode == EtiMode.BASIC:
                self.processExtra()
    ...
    def _process_namegen(self) -> str:
        ...
        args_position = ""
        if self.eti_mode == EtiMode.DERIV:
            args_position = "deriv"
        return f'python3 {dir_script}/../../libs/namegen/namegen.py {args_position} --def-lang {self.lang} --include-no-morphs --error-words {self.outdir}/ma_unknown_words_{self.eti_mode}.lntrf -o "{self.outfile}" "{self.infile}"'

    def _process_extra_namegen(self) -> None:
        for type_flag, fn_label in {
            "G": "given_names",
            "L": "locations",
            "S": "surnames",
        }.items():
            fn_out = f"{self.outdir}/ma_suggested_{fn_label}.lntrf"
            cmd = f"grep -P '\tj{type_flag}' {self.outdir}/ma_unknown_words_{self.eti_mode}.lntrf"
            self._run_cmd_save_output_to_file(cmd=cmd, file_path=fn_out)

            for lang, re_pattern in {
                self.lang: re.escape(self.lang),
                "unknown": "",
            }.items():
        ...
        with open(file_path, "w") as f_out:
            try:
                ...
                if e.returncode != 1:
                    ...
                    print(
                        f"ERROR in name suggestions: {err_msg_base} failed with return code {e.returncode}.{err_output_detail}\n(command: {cmd})",
                        file=sys.stderr,
                        flush=True,
                    )
                    sys.exit(11)
